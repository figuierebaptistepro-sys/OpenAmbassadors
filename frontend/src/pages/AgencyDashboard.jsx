...
import {
  Plus, Search, RefreshCw, AlertCircle, Video, Users,
  TrendingUp, ChevronRight, Film, FileText, Link2,
  Copy, Building2, Clock, CheckCircle, ArrowRight
} from "lucide-react";
...
export default function AgencyDashboard({ user }) {
  ...
  // "Needs attention" — scripts with modifications requested
    ...
    <AppLayout user={user}>
      <div className="h-[calc(100vh-56px)] lg:h-screen flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 bg-white border-b border-gray-200 px-5 py-3 flex items-center gap-3">
          <div>
            <h1 className="font-bold text-gray-900 text-sm leading-none">Agence Studiosavora</h1>
            {stats && (
              <p className="text-xs text-gray-400 mt-0.5">
                {stats.clients_actifs} clients · {stats.campagnes_actives} actives · {stats.scripts_en_attente} scripts à valider
              </p>
            )}
          </div>
          <div className="flex-1 relative max-w-xs">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
            <Input className="pl-8 h-8 text-sm border-gray-200 bg-gray-50" placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div className="ml-auto flex gap-2">
            <Button size="sm" variant="outline" onClick={generateInvite} className="h-8 text-xs border-gray-200">
              <Link2 className="w-3.5 h-3.5 mr-1" /> Inviter client
            </Button>
            <Button size="sm" onClick={() => navigate("/agency/campaign/new")} className="h-8 text-xs bg-[#FF2E63] hover:bg-[#e0254f] text-white">
              <Plus className="w-3.5 h-3.5 mr-1" /> Nouvelle campagne
            </Button>
          </div>
        </div>
        ...
        {needsAttention.length > 0 && (
          <div className="flex-shrink-0 bg-orange-50 border-b border-orange-200 px-5 py-2 flex items-center gap-2 overflow-x-auto">
            ...
            {STATUSES.map(st => {
              ...
              return (
                <div key={st.key} className="flex flex-col w-64 flex-shrink-0">
                  ...
                  {/* Cards */}
                  <div className={`flex-1 overflow-y-auto border-x border-b ${st.color} rounded-b-xl bg-white/60 p-2 space-y-2 min-h-0`}>
                    {cols.length === 0 && (
                      <div className="text-center py-8 text-xs text-gray-300">Vide</div>
                    )}
                    {cols.map(c => <CampaignCard key={c.campaign_id} campaign={c} status={st} onOpen={() => navigate(`/agency/campaign/${c.campaign_id}`)} onMove={moveStatus} moving={movingId === c.campaign_id} statuses={STATUSES} />)}
                  </div>
                </div>
              );
            })}
...
function CampaignCard({ campaign: c, status: st, onOpen, onMove, moving, statuses }) {
  const formula = FORMULAS.find(f => f.key === c.formula);
  ...
  return (
    <div onClick={onOpen} className={`bg-white rounded-xl border border-gray-200 p-3 cursor-pointer hover:shadow-md hover:border-gray-300 transition-all group ${moving ? "opacity-50" : ""}`}>
      {/* Client + title */}
      <div className="mb-2">
